...
if ~isfield(pars,'display_images')	pars.display_images = true; end; %false;
...
if ~exist('Binit','var') || isempty(Binit)
    ...
    sR = zeros(sqrt(pars.patch_size/3),sqrt(pars.patch_size/3),3);
    sR(:,:,1) = 255*ones(sqrt(pars.patch_size/3));
    B(:,1)= reshape(rgb2lab2mat(sR),pars.patch_size,1);
    
    sG = zeros(sqrt(pars.patch_size/3),sqrt(pars.patch_size/3),3);
    sG(:,:,2) = 255*ones(sqrt(pars.patch_size/3));
    B(:,2)= reshape(rgb2lab2mat(sG),pars.patch_size,1);
    
    sB = zeros(sqrt(pars.patch_size/3),sqrt(pars.patch_size/3),3);
    sB(:,:,3) = 255*ones(sqrt(pars.patch_size/3));
    B(:,3)= reshape(rgb2lab2mat(sB),pars.patch_size,1);
else
    disp('Using Binit...');
    B = Binit;
end;
...
while t < pars.num_trials
    ...
    if exist('resample_size', 'var') && resample_size
        ...
        X = X_total(:, randsample(size(X_total,2), resample_size));%TODO
    end
